...
resource "aws_iam_user" "creating_user" {
  count = length(var.arrayvar)
  name  = "${element(var.arrayvar,count.index)}"
}

resource "aws_instance" "ashuvm" {
  ami                         = var.ami-details
  instance_type               = var.inst_details
  key_name                    = aws_key_pair.new-key.key_name
  count                       = var.count1
  associate_public_ip_address = var.bool1
